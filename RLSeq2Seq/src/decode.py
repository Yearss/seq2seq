...
class BeamSearchDecoder(object):
    ...
    def decode(self):
        ...
        while True:
            ...
            title_withunks = data.show_abs_oovs(original_title, self._vocab,
            ...
            #print(original_title.encode("utf-8"))
            if FLAGS.single_pass:
                self.write_for_rouge(original_title, decoded_words,
                                     counter)  # write ref summary and decoded summary to file, to eval with pyrouge later
                counter += 1  # this is how many examples we've decoded
            else:
                print_results(article_withunks, title_withunks, decoded_output)  # log output to screen
                self.write_for_attnvis(article_withunks, title_withunks, decoded_words, best_hyp.attn_dists,
    ...
    def write_for_rouge(self, reference_sents, decoded_words, ex_index):
        ...
        with open(ref_file, "wb") as f:
            for idx, sent in enumerate(reference_sents):
                f.write(sent.encode("utf-8")) if idx == len(reference_sents) - 1 else f.write(sent.encode("utf-8") + "\n")
        with open(decoded_file, "wb") as f:
            for idx, sent in enumerate(decoded_sents):
                f.write(sent.encode("utf-8")) if idx == len(decoded_sents) - 1 else f.write(sent.encode("utf-8") + "\n")
...
def get_decode_dir_name(ckpt_name):
    ...
    dirname = "decode_%s_%s_%imaxenc_%ibeam_%imindec_%imaxdec" % (
    dataset, FLAGS.decode_from, FLAGS.max_enc_steps, FLAGS.beam_size, FLAGS.min_dec_steps, FLAGS.max_dec_steps)
