...
class Example(object):
    ...
    def __init__(self, article, title, vocab, hps):
        ...
        # Process the title
        title_words = title.split()  # list of strings
        title_ids = [vocab.word2id(w) for w in
                     title_words]  # list of word ids; OOVs are represented by the id for UNK token
        self.dec_input, self.target = self.get_dec_inp_targ_seqs(title_ids, hps.max_dec_steps, start_decoding,
                                                                 stop_decoding)
        ...
        if hps.pointer_gen:
            ...
            # Get a version of the reference summary where in-article OOVs are represented
            # by their temporary article OOV id
            title_ids_extend_vocab = data.title2ids(title_words, vocab, self.article_oovs)

            # Overwrite decoder target sequence so it uses the temp article OOV ids
            _, self.target = self.get_dec_inp_targ_seqs(title_ids_extend_vocab, hps.max_dec_steps, start_decoding,
                                                        stop_decoding)
...
class Batch(object):
    ...
    def store_orig_strings(self, example_list):
        ...
        self.original_titles = [ex.original_title for ex in example_list]  # list of lists
...
class Batcher(object):
    ...
    def fill_example_queue(self):
        ...
        while True:
            try:
                (article,
                 title) = input_gen.next()  # read the next example from file. article and abstract are both strings.
            ...
            example = Example(article, title, self._vocab, self._hps)  # Process into an Example.
        ...
        while True:
            ...
            try:
                article_text = e.features.feature['article'].bytes_list.value[
                    0].decode("utf-8")  # the article text was saved under the key 'article' in the data files
                abstract_text = e.features.feature['abstract'].bytes_list.value[
                    0].decode("utf-8")  # the abstract text was saved under the key 'abstract' in the data files
            except ValueError:
