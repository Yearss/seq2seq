...
tf.app.flags.DEFINE_boolean('single_pass', False,
                            ...
                            'checkpoint, use it to produce summaries for randomly-chosen examples and log the results to screen, indefinitely.')
...
tf.app.flags.DEFINE_boolean('share_decoder_weights', False,
                            'Share output matrix projection with word embedding')  # Eq 13. in https://arxiv.org/pdf/1705.04304.pdf

# Pointer-generator with Self-Critic policy gradient: https://arxiv.org/pdf/1705.04304.pdf
tf.app.flags.DEFINE_boolean('rl_training', False,
                            'Use policy-gradient training by collecting rewards at the end of sequence.')
...
tf.app.flags.DEFINE_boolean('convert_to_reinforce_model', False,
                            'Convert a pointer model to a reinforce model. Turn this on and run in train mode. Your current training model will be copied to a new version (same name with _cov_init appended) that will be ready to run with coverage flag turned on, for the coverage training stage.')
...
tf.app.flags.DEFINE_boolean('matrix_attention', False,
                            'Use matrix attention, Eq. 2 https://arxiv.org/pdf/1705.04304.pdf')
...
tf.app.flags.DEFINE_string('reward_function', 'rouge_l/f_score',
                           'either bleu or one of the rouge measures (rouge_1/f_score,rouge_2/f_score,rouge_l/f_score)')

# parameters of DDQN model
tf.app.flags.DEFINE_boolean('ac_training', False, 'Use Actor-Critic learning by DDQN.')
tf.app.flags.DEFINE_boolean('dqn_scheduled_sampling', False,
                            'Whether to use scheduled sampling to use estimates of dqn model vs the actual q-estimates values')
tf.app.flags.DEFINE_string('dqn_layers', '512,256,128',
                           'DQN dense hidden layer size, will create three dense layers with 512, 256, and 128 size')
...
tf.app.flags.DEFINE_boolean('dueling_net', True,
                            'Whether to use Duelling Network to train the model')  # https://arxiv.org/pdf/1511.06581.pdf
tf.app.flags.DEFINE_boolean('dqn_polyak_averaging', True,
                            'Whether to use polyak averaging to update the target network parameters')
tf.app.flags.DEFINE_boolean('calculate_true_q', False,
                            "Whether to use true Q-values to train DQN or use DQN's estimates to train it")
...
tf.app.flags.DEFINE_boolean('fixed_sampling_probability', False,
                            'Whether to use fixed sampling probability or adaptive based on global step')
tf.app.flags.DEFINE_boolean('hard_argmax', True, 'Whether to use soft argmax or hard argmax')
tf.app.flags.DEFINE_boolean('greedy_scheduled_sampling', False,
                            'Whether to use greedy approach or sample for the output, if True it uses greedy')
...
tf.app.flags.DEFINE_boolean('scheduled_sampling_final_dist', True,
                            'Whether to use final distribution or vocab distribution for scheduled sampling')
...
class Seq2Seq(object):
    ...
    def convert_to_coverage_model(self):
        ...
        saver = tf.train.Saver(
            [v for v in tf.global_variables() if "coverage" not in v.name and "Adagrad" not in v.name])
    ...
    def convert_to_reinforce_model(self):
        ...
        saver = tf.train.Saver(
            [v for v in tf.global_variables() if "reinforce" not in v.name and "Adagrad" not in v.name])
    ...
    def setup_training(self):
        ...
        self.sv = tf.train.Supervisor(logdir=train_dir,
                                      ...
                                      init_feed_dict={
                                          self.model.embedding_place: word_vector} if FLAGS.embedding else None
                                      )
    ...
    def run_training(self):
        ...
        while True:  # repeats until interrupted
            ...
            if FLAGS.ac_training:
                ...
                transitions = self.model.collect_dqn_transitions(self.sess, batch, self.train_step,
                ...
                with self.dqn_graph.as_default():
                    ...
                    b = ReplayBuffer.create_batch(self.dqn_hps, transitions, len(transitions), use_state_prime=False,
                                                  max_art_oovs=batch.max_art_oovs)
                    # we also get the next decoder state to correct the estimation, use_state_prime = True
                    b_prime = ReplayBuffer.create_batch(self.dqn_hps, transitions, len(transitions),
                                                        use_state_prime=True, max_art_oovs=batch.max_art_oovs)
                    ...
                    q_estimates = np.concatenate([q_estimates,
                                                  np.reshape(q_estimates[:, 0], [-1, 1]) * np.ones(
                                                      (len(transitions), batch.max_art_oovs))], axis=-1)
                    ...
                    if FLAGS.dqn_scheduled_sampling:
                        q_estimates = self.scheduled_sampling(batch_len, FLAGS.sampling_probability, b._y_extended,
                                                              q_estimates)
                    ...
                    q_estimates = np.reshape(q_estimates, [FLAGS.batch_size, FLAGS.k, FLAGS.max_dec_steps,
                                                           -1])  # shape (batch_size, k, max_dec_steps, vocab_size_extended)
    ...
    def dqn_training(self):
        ...
        try:
            while True:
                ...
                dqn_target_results = self.dqn_target.run_test_steps(self.dqn_sess, x=dqn_batch._x, y=dqn_batch._y,
                                                                    return_loss=True)
                self.dqn_train_step = dqn_results['global_step']
                self.dqn_summary_writer.add_summary(dqn_results['summaries'],
                                                    self.dqn_train_step)  # write the summaries
                ...
                with self.dqn_graph.as_default():
                    current_model_weights = self.dqn_sess.run([self.dqn.model_trainables])[
                        0]  # get weights of current model
                    self.dqn_target.run_update_weights(self.dqn_sess, self.dqn_train_step,
                                                       current_model_weights)  # update target model weights with current model weights
                tf.logging.info('DQN loss at step {}: {}'.format(self.dqn_train_step, np.mean(self.avg_dqn_loss)))
                tf.logging.info(
                    'DQN Target loss at step {}: {}'.format(self.dqn_train_step, np.mean(avg_dqn_target_loss)))
    ...
    def run_eval(self):
        ...
        while True:
            ...
            while processed_batch < 100 * FLAGS.batch_size:
                ...
                if FLAGS.ac_training:
                    t0 = time.time()
                    transitions = self.model.collect_dqn_transitions(sess, batch, train_step,
                                                                     batch.max_art_oovs)  # len(batch_size * k * max_dec_steps)
                    tf.logging.info('Q values collection time: {}'.format(time.time() - t0))
                    with dqn_graph.as_default():
                        ...
                        batch_len = len(transitions)
                        b = ReplayBuffer.create_batch(self.dqn_hps, transitions, len(transitions), use_state_prime=True,
                                                      max_art_oovs=batch.max_art_oovs)
                        b_prime = ReplayBuffer.create_batch(self.dqn_hps, transitions, len(transitions),
                                                            use_state_prime=True, max_art_oovs=batch.max_art_oovs)
                        ...
                        # we need to expand the q_estimates to match the input batch max_art_oov
                        q_estimates = np.concatenate([q_estimates, np.zeros((len(transitions), batch.max_art_oovs))],
                                                     axis=-1)
                        ...
                        for i, tr in enumerate(transitions):
                            if tr.done:
                                ...
                                q_estimates[i][tr.action] = tr.reward + FLAGS.gamma * q_vals_new_t[i][
                                    dqn_best_action[i]]
                        if FLAGS.dqn_scheduled_sampling:
    ...
    def main(self, unused_argv):
        ...
        self.batcher = Batcher(FLAGS.data_path, self.vocab, self.hps, single_pass=FLAGS.single_pass,
