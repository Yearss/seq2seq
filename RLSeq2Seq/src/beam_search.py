...
def run_beam_search(sess, model, vocab, batch, dqn=None, dqn_sess=None, dqn_graph=None):
    ...
    while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
        ...
        decoder_outputs = np.array([h.decoder_output for h in hyps]).swapaxes(0, 1)
        encoder_es = np.array([h.encoder_mask for h in hyps]).swapaxes(0, 1)

        # Run one step of the decoder to get the new info
        (topk_ids, topk_log_probs, new_states, attn_dists, final_dists, p_gens, new_coverage, decoder_output,
         encoder_e) = model.decode_onestep(sess=sess,
                                           batch=batch,
        ...
        if FLAGS.ac_training:
            with dqn_graph.as_default():
                ...
                q_estimates = np.concatenate([q_estimates, np.reshape(q_estimates[:, 0], [-1, 1]) * np.ones(
                    (FLAGS.beam_size, batch.max_art_oovs))], axis=-1)
