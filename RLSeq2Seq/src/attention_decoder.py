...
def attention_decoder(_hps,
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
            ...
            with variable_scope.variable_scope("Attention"):
                ...
                if not _hps.matrix_attention and use_coverage and coverage is not None:  # non-first step of coverage
                    ...
                    coverage_features = nn_ops.conv2d(coverage, w_c, [1, 1, 1, 1],
                                                      "SAME")  # c has shape (batch_size, max_enc_steps, 1, attention_vec_size)
                    ...
                    if _hps.use_temporal_attention:
                        ...
                        if len_temporal_e == 0:
                            ...
                            masked_sums = tf.reduce_sum(temporal_e,
                                                        axis=0) + 1e-10  # if it's zero due to masking we set it to a small value
                    ...
                    if _hps.matrix_attention:
                        ...
                        _dec_attn = tf.unstack(tf.matmul(tf.squeeze(decoder_features, axis=[1, 2]), w_attn),
                                               axis=0)  # batch_size * (attention_vec_size)
                        _enc_states_lst = tf.unstack(tf.squeeze(_enc_states, axis=2),
                                                     axis=0)  # batch_size * (max_enc_steps, attention_vec_size)

                        e_not_masked = tf.squeeze(tf.stack(
                            [tf.matmul(tf.reshape(_dec, [1, -1]), tf.transpose(_enc)) for _dec, _enc in
                             zip(_dec_attn, _enc_states_lst)]), axis=1)  # (batch_size, max_enc_steps)
                ...
                context_vector = math_ops.reduce_sum(array_ops.reshape(attn_dist, [batch_size, -1, 1, 1]) * _enc_states,
        ...
        def intra_decoder_attention(decoder_state, outputs):
            ...
            try:
                ...
                len_dec_states = 0
            attention_dec_vec_size = attn_dec_size = decoder_state.c.get_shape()[1]  # hidden_dim
            _decoder_states = tf.expand_dims(tf.reshape(outputs, [batch_size, -1, attn_dec_size]),
                                             axis=2)  # now is shape (batch_size,len(decoder_states), 1, attn_size)
            _prev_decoder_features = nn_ops.conv2d(_decoder_states, W_h_d, [1, 1, 1, 1],
                                                   "SAME")  # shape (batch_size,len(decoder_states),1,attention_vec_size)
            with variable_scope.variable_scope("DecoderAttention"):
                # Pass the decoder state through a linear layer (this is W_s s_t + b_attn in the paper)
                try:
                    decoder_features = linear(decoder_state, attention_dec_vec_size,
                                              True)  # shape (batch_size, attention_vec_size)
                    decoder_features = tf.expand_dims(tf.expand_dims(decoder_features, 1),
                                                      1)  # reshape to (batch_size, 1, 1, attention_dec_vec_size)
                    # Calculate v^T tanh(W_h h_i + W_s s_t + b_attn)
                    if _hps.matrix_attention:
                        # Calculate h_d * W_attn * h_d, equation 6 in https://arxiv.org/pdf/1705.04304.pdf
                        _dec_attn = tf.matmul(tf.squeeze(decoder_features),
                                              w_dec_attn)  # (batch_size, decoder_attn_size)
                        _dec_states_lst = tf.unstack(tf.reshape(_prev_decoder_features, [batch_size, -1,
                                                                                         decoder_attn_size]))  # batch_size * (len(decoder_states), decoder_attn_size)
                        e_not_masked = tf.reshape(
                            tf.stack([tf.matmul(_dec_attn, tf.transpose(k)) for k in _dec_states_lst]),
                            [batch_size, -1])  # (batch_size, len(decoder_states))
                        masked_e = tf.exp(
                            e_not_masked * dec_padding_mask[:, :len_dec_states])  # (batch_size, len(decoder_states))
                    else:
                        # Calculate v^T tanh(W_h h_i + W_s s_t + b_attn)
                        e_not_masked = math_ops.reduce_sum(
                            v_d * math_ops.tanh(_prev_decoder_features + decoder_features),
                            [2, 3])  # calculate e, (batch_size,len(decoder_states))
                        masked_e = nn_ops.softmax(e_not_masked) * dec_padding_mask[:,
        ...
        if initial_state_attention:  # true in decode mode
            ...
            context_vector, _, coverage, _ = attention(_dec_in_state, tf.stack(prev_encoder_es, axis=0),
                                                       coverage)  # in decode mode, this is what updates the coverage vector
            if _hps.intradecoder:
                context_decoder_vector, _ = intra_decoder_attention(_dec_in_state,
                                                                    tf.stack(prev_decoder_outputs, axis=0))

        # Add decoder for every step
        for i, inp in enumerate(emb_dec_inputs):
            ...
            if _hps.mode in ['train',
                             'eval'] and _hps.scheduled_sampling and i > 0:  # start scheduled sampling after we received the first decoder's output
            ...
            if i == 0 and initial_state_attention:  # always true in decode mode
                with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                    ...
                    context_vector, attn_dist, _, masked_e = attention(state, tf.stack(prev_encoder_es, axis=0),
                                                                       coverage)  # don't allow coverage to update
                    if _hps.intradecoder:
                        context_decoder_vector, _ = intra_decoder_attention(state,
                                                                            tf.stack(prev_decoder_outputs, axis=0))
            else:
            ...
            with tf.variable_scope('output_projection'):
                ...
                vocab_dists.append(
                    vocab_dist)  # The vocabulary distributions. List length max_dec_steps of (batch_size, vsize) arrays. The words are in the order they appear in the vocabulary file.
            ...
            if FLAGS.use_discounted_rewards:
                ...
                for _ in range(_hps.k):
                    ...
                    rl_fscore = tf.reshape(
                        rouge_l_fscore(tf.transpose(tf.stack(greedy_search_samples)[:, :, _]), target_batch),
                        [-1, 1])  # shape (batch_size, 1)
    ...
    return (
        outputs, state, attn_dists, p_gens, coverage, vocab_scores, final_dists, samples, greedy_search_samples,
        temporal_e,
...
def scheduled_sampling(hps, sampling_probability, output, embedding, inp, alpha=0):
    ...
    with variable_scope.variable_scope("ScheduledEmbedding"):
        ...
        sample_id_sampler = categorical.Categorical(
            probs=output)  # equals to argmax{ Multinomial(output, total_count=1) }, our greedy search selection
        ...
        if hps.E2EBackProp:
            if hps.hard_argmax:
                ...
                normalized_embedding = tf.multiply(
                    tf.reshape(greedy_search_prob_normalized, [hps.batch_size, hps.k, 1]), greedy_embedding)
                ...
                greedy_search_prob_normalized = greedy_search_prob / tf.reshape(
                    tf.reduce_sum(greedy_search_prob, axis=1),
                    [-1, 1])
